 ...
 * <p>
 * Each instance of this object defines a DHCPv6 server pool. Client requests
 * are associated with pools based on criteria such as source interface,
 * supplied DHCPv6 options, and source address.
 * <p>
 * Overlapping pool ranges MUST be supported.
 * <p>
 * For enabled table entries, if {{param|Interface}} is not a valid reference to
 * an IPv6-capable interface (that is attached to the IPv6 stack) then the table
 * entry is inoperable and the CPE MUST set {{param|Status}} to
 * {{enum|Error_Misconfigured|Status}}.
 *
 * @since TR181 v2.2
 */
@CWMPObject(name = "Device.DHCPv6.Server.Pool.{i}.", uniqueConstraints = {
...
public class Pool {
   ...
   * ''1'' indicates the first entry considered (highest precedence). For each
   * DHCPv6 request, the highest ordered entry that matches the association
   ...
   * entry, the {{param}} value for the existing entry and all lower {{param}}
   * entries is incremented (lowered in precedence) to ensure uniqueness of this
   * value. A deletion causes {{param}} values to be compacted. When a value is
   * changed, incrementing occurs before compaction.
   * <p>
   * The value of {{param}} on creation of a {{object}} table entry MUST be one
   * greater than the largest current value (initially assigned the lowest
   * precedence).
   *
   * @since 2.2
   */
   ...
   * <p>
   * A hexbinary string used to identify one or more LAN devices, value of the
   * DHCPv6 Client Identifier (Option 1) {{bibref|RFC3315|Section 22.2}}. The
  ...
  public byte[] duiD;
  /**
   * If {{false}}, matching packets are those that match the {{param|DUID}}
   * entry, if specified.
   * <p>
   ...
   * <p>
   * A hexbinary string used to identify one or more LAN devices, value of the
   * DHCPv6 Vendor Class Identifier (Option 16) {{bibref|RFC3315|Section
   * 22.16}}. The option value is binary, so an exact match is REQUIRED.
   * <p>
   * {{empty}} indicates this criterion is not used.
   *
   * @since 2.2
   ...
   * <p>
   * A hexbinary string used to identify one or more LAN devices, value of the
   * DHCPv6 User Class Identifier (Option 15) {{bibref|RFC3315|Section 22.15}}.
   * <p>
   * {{empty}} indicates this criterion is not used.
   *
   * @since 2.2
   ...
   * Pool association criterion.
   * <p>
   * Source address (link-layer address) of DHCPv6 messages sent from the DHCPv6
   * client. This criterion is only likely to be useful if this is a link-layer
   * address.
   * <p>
   ...
   * positions set to one in the mask. A mask of {{empty}} indicates all bits of
   * the {{param|SourceAddress}} are to be used.
   *
   * @since 2.2
   */
   ...
   * <p>
   * If {{true}}, matching packets are those that do not match the (masked)
   * {{param|SourceAddress}} entry, if specified.
   *
   * @since 2.2
   */
   ...
   * {{list}} {{reference}}
   * <p>
   * Manually-configured prefixes from which IA_NA addresses will be assigned.
   * Each referenced prefix MUST have a
   * {{param|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}} of
   * {{enum|Static|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}} or
   * {{enum|Child|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}}. All clients
   * that request IA_NA and match filter criteria on this {{param|Interface}}
   * MUST be offered IA_NA addresses from all of the ''Valid''
   * ({{param|.IP.Interface.{i}.IPv6Prefix.{i}.ValidLifetime}} is infinite or in
   * the future) /64 prefixes in this list.
   * <p>
   ...
   * {{list}} {{reference}}
   * <p>
   * All prefixes from which IA_NA addresses will be assigned. This list can
   * include:
   * <p>
   ...
   * {{enum|AutoConfigured|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} associated
   ...
   * {{list}} {{reference}}
   * <p>
   * Manually-configured prefixes from which IA_PD prefixes will be derived.
   * This list can include:
   * <p>
   * Prefixes with {{param|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} =
   * {{enum|Static|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} or
   * {{enum|PrefixDelegation|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}}
   * associated with upstream interfaces (i.e. interfaces for which the physical
   * layer interface object has ''Upstream'' = {{true}}).
   * <p>
   * Prefixes with {{param|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}} =
   * {{enum|Static|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}} or
   * {{enum|Child|.IP.Interface.{i}.IPv6Prefix.{i}.StaticType}} associated with
   * with the interface instance referenced by {{param|Interface}}.
   * <p>
   * All clients that request IA_PD and match filter criteria on this
   * {{param|Interface}} MUST be offered IA_PD prefixes derived from all of the
   * ''Valid'' ({{param|.IP.Interface.{i}.IPv6Prefix.{i}.ValidLifetime}} is
   ...
   * <p>
   * Prefixes with {{param|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} =
   * {{enum|PrefixDelegation|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} or
   * {{enum|AutoConfigured|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} associated
   * with upstream interfaces (i.e. interfaces for which the physical layer
   * interface object has ''Upstream'' = {{true}}).
   * <p>
   * Prefixes with {{param|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} =
   * {{enum|Child|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} or
   * {{enum|AutoConfigured|.IP.Interface.{i}.IPv6Prefix.{i}.Origin}} associated
   ...
   * determine the length of prefixes that are offered in an IA_PD. The device
   * is responsible for ensuring that it does not delegate a prefix longer than
   * /64. The device can have additional logic that is used (in conjunction with
   * this parameter) to determine the actual length of prefixes offered in an
   * IA_PD.
   *
   * @since 2.2
   */
   ...
   * DHCPv6 client table.
   * <p>
   * This table lists details of DHCPv6 clients that matched the filter criteria
   * of this {{object|#}} entry.
   */
  @XmlElementWrapper(name = "Clients")
  @XmlElement(name = "Client")
  public Collection<Client> clients;
  /**
   * This object specifies the DHCPv6 options that MUST, if enabled, be offered
   * to clients whose DHCPv6 requests are associated with this pool. If
   * {{param|PassthroughClient}} is specified, and the referenced client has a
   * value for a given option then the {{param|PassthroughClient}} option value
   * will be sent instead of {{param|Value}}. Otherwise, {{param|Value}} will be
   * sent.
   */
  ...
  /**
   * Get the position of the {{object}} entry in the order of precedence. A
   * value of ''1'' indicates the first entry considered (highest precedence).
  ...
  /**
   * Set the position of the {{object}} entry in the order of precedence. A
   * value of ''1'' indicates the first entry considered (highest precedence).
   * For each DHCPv6 request, the highest ordered entry that matches the
   * association criteria is applied. All lower order entries are ignored.
   * <p>
  ...
  /**
   * Get the if {{false}}, matching packets are those that match the
   * {{param|DUID}} entry, if specified.
   * <p>
   * If {{true}}, matching packets are those that do not match the
   * {{param|DUID}} entry, if specified.
  ...
  /**
   * Set the if {{false}}, matching packets are those that match the
   * {{param|DUID}} entry, if specified.
   * <p>
   * If {{true}}, matching packets are those that do not match the
   * {{param|DUID}} entry, if specified.
  ...
  /**
   * Get the bit-mask for the source address, where matching of a packet's
   * source address with the {{param|SourceAddress}} is only to be done for bit
   * positions set to one in the mask. A mask of {{empty}} indicates all bits of
   * the {{param|SourceAddress}} are to be used.
   *
   * @since 2.2
  ...
  /**
   * Set the bit-mask for the source address, where matching of a packet's
   * source address with the {{param|SourceAddress}} is only to be done for bit
   * positions set to one in the mask. A mask of {{empty}} indicates all bits of
   * the {{param|SourceAddress}} are to be used.
   *
  ...
  /**
   * Get the if {{false}}, matching packets are those that match the (masked)
   * {{param|SourceAddress}} entry, if specified.
   * <p>
   * If {{true}}, matching packets are those that do not match the (masked)
   * {{param|SourceAddress}} entry, if specified.
  ...
  /**
   * Set the if {{false}}, matching packets are those that match the (masked)
   * {{param|SourceAddress}} entry, if specified.
   * <p>
   * If {{true}}, matching packets are those that do not match the (masked)
   * {{param|SourceAddress}} entry, if specified.
  ...
  public Collection<String> getIanAManualPrefixes() {
    if (this.ianAManualPrefixes == null) {
      this.ianAManualPrefixes = new ArrayList<>();
    }
  ...
  public Collection<String> getIanAPrefixes() {
    if (this.ianAPrefixes == null) {
      this.ianAPrefixes = new ArrayList<>();
    }
    return ianAPrefixes;
  }
  ...
  public Collection<String> getIapDManualPrefixes() {
    if (this.iapDManualPrefixes == null) {
      this.iapDManualPrefixes = new ArrayList<>();
    }
  ...
  public Collection<String> getIapDPrefixes() {
    if (this.iapDPrefixes == null) {
      this.iapDPrefixes = new ArrayList<>();
    }
    return iapDPrefixes;
  }
  ...
  /**
   * Get the RECOMMENDED minimum number of bits to add to
   * {{param|IAPDManualPrefixes}} or {{param|IAPDPrefixes}} in order to
   * determine the length of prefixes that are offered in an IA_PD. The device
   * is responsible for ensuring that it does not delegate a prefix longer than
   * /64. The device can have additional logic that is used (in conjunction with
  ...
  /**
   * Set the RECOMMENDED minimum number of bits to add to
   * {{param|IAPDManualPrefixes}} or {{param|IAPDPrefixes}} in order to
   * determine the length of prefixes that are offered in an IA_PD. The device
   * is responsible for ensuring that it does not delegate a prefix longer than
   * /64. The device can have additional logic that is used (in conjunction with
  ...
  /**
   * Get the this object specifies the DHCPv6 options that MUST, if enabled, be
   * offered to clients whose DHCPv6 requests are associated with this pool. If
   * {{param|PassthroughClient}} is specified, and the referenced client has a
   * value for a given option then the {{param|PassthroughClient}} option value
   * will be sent instead of {{param|Value}}. Otherwise, {{param|Value}} will be
   * sent.
  ...
  /**
   * Set the this object specifies the DHCPv6 options that MUST, if enabled, be
   * offered to clients whose DHCPv6 requests are associated with this pool. If
   * {{param|PassthroughClient}} is specified, and the referenced client has a
   * value for a given option then the {{param|PassthroughClient}} option value
   * will be sent instead of {{param|Value}}. Otherwise, {{param|Value}} will be
