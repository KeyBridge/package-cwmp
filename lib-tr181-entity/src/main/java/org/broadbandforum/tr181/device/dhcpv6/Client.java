 ...
 * <p>
 * For enabled table entries, if {{param|Interface}} is not a valid reference to
 * an IPv6-capable interface (that is attached to the IPv6 stack), then the
 * table entry is inoperable and the CPE MUST set {{param|Status}} to
 * {{enum|Error_Misconfigured|Status}}.
 * <p>
 * Note: The {{object}} table includes a unique key parameter that is a strong
 * reference. If a strongly referenced object is deleted, the CPE will set the
 * referencing parameter to {{empty}}. However, doing so under these
 * circumstances might cause the updated {{object}} row to then violate the
 * table's unique key constraint; if this occurs, the CPE MUST set
 * {{param|Status}} to {{enum|Error_Misconfigured|Status}} and disable the
 * offending {{object}} row.
 *
 * @since TR181 v2.2
 */
@CWMPObject(name = "Device.DHCPv6.Client.{i}.", uniqueConstraints = {
...
public class Client {
  ...
  public Alias alias;
  /**
   * {{reference}} The IP Interface associated with the {{object}} entry. This
   * will reference an IPv6-capable interface (that is attached to the IPv6
  ...
  /**
   * The client's DHCP Unique Identifier (DUID) {{bibref|RFC3315|Section 9}}.
   * {{param}} is set by the CPE.
   *
   * @since 2.2
   */
   ...
   * Non-Temporary Address'' option OPTION_IA_NA(3) {{bibref|RFC3315|Section
   * 22.4}} in Solicit messages.
   *
   * @since 2.2
   */
   ...
   * Delegation'' option OPTION_IA_PD(25) {{bibref|RFC3633|Section 10}} in
   * Solicit messages.
   * <p>
   ...
   * information (i.e. the CPE will do a renew or information request as needed,
   * updating both stateful and stateless parameter values discovered by this
   * Client instance).
   *
   * @since 2.2
   */
  ...
  /**
   * T1 value, in seconds, that the client SHOULD use when sending IA options,
   * e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
  ...
  /**
   * T2 value, in seconds, that the client SHOULD use when sending IA options,
   * e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
   * {{bibref|RFC3633|Section 10}}.
   * <p>
   ...
   * responses. This list MUST include both top-level and encapsulated options,
   * e.g. if the client is able to process OPTION_IA_NA (3) with an encapsulated
   * OPTION_IAADDR (5), the list would be expected to include both 3 and 5.
   *
   * @since 2.2
   */
  ...
  /**
   * {{list}} An ordered list of the top-level options (i.e. not encapsulated
   * options) that the client will explicitly request from the server.
   * <p>
   ...
   * example, it is appropriate for OPTION_DNS_SERVERS {{bibref|RFC3646}} but is
   * not appropriate for OPTION_SERVERID (which is part of the protocol),
   * OPTION_IA_NA (which is modeled via {{param|RequestAddresses}}) or
   * OPTION_IA_PD (which is modeled via {{param|RequestPrefixes}}). However, the
   * CPE MUST NOT reject an attempt to set this parameter to a value that
   * includes options that it regards as inappropriate; instead, it MUST accept
   * the requested options and handle them as best it can.
   * <p>
   * Loosely speaking, these are the options that the client will include in
   * OPTION_ORO {{bibref|RFC3315}} but the client is free to decide (in
   * accordance with {{bibref|RFC3315}}) in which messages to request which
   ...
   * also in {{param|SupportedOptions}}.
   *
   * @since 2.2
   */
   ...
   * does ''not'' model a local DHCP server). Table entries are automatically
   * created to correspond with these servers. However, it is a local matter to
   * the CPE when to delete old table entries.
   */
   ...
   * operation of the protocol, and whose values are simple, do not often change
   * and are not modeled elsewhere. For example, it is appropriate for
   ...
   * received options. However, it is a local matter to the CPE when to delete
   * old table entries.
   * <p>
   ...
   * whether the same option will be present multiple times). In order to allow
   * for the same option to be present multiple times within the table, this
   * table has no unique key defined.
   */
  ...
  /**
   * Get the {{reference}} The IP Interface associated with the {{object}}
   * entry. This will reference an IPv6-capable interface (that is attached to
  ...
  /**
   * Set the {{reference}} The IP Interface associated with the {{object}}
   * entry. This will reference an IPv6-capable interface (that is attached to
   * the IPv6 stack), otherwise the table entry will be inoperable.
   *
   * @since 2.2
  ...
  /**
   * Get the client's DHCP Unique Identifier (DUID) {{bibref|RFC3315|Section
  ...
  /**
   * Set the client's DHCP Unique Identifier (DUID) {{bibref|RFC3315|Section
   * 9}}. {{param}} is set by the CPE.
   *
   * @since 2.2
  ...
  /**
   * Get the enables or disables inclusion of the ''Identity Association (IA)
   * for Non-Temporary Address'' option OPTION_IA_NA(3) {{bibref|RFC3315|Section
   * 22.4}} in Solicit messages.
   *
   * @since 2.2
   * @return the value
  ...
  /**
   * Set the enables or disables inclusion of the ''Identity Association (IA)
   * for Non-Temporary Address'' option OPTION_IA_NA(3) {{bibref|RFC3315|Section
   * 22.4}} in Solicit messages.
   *
   * @since 2.2
  ...
  /**
   * Get the enables or disables inclusion of the ''Identity Association (IA)
   * for Prefix Delegation'' option OPTION_IA_PD(25) {{bibref|RFC3633|Section
  ...
  /**
   * Get the enables or disables inclusion of the ''Rapid Commit'' option
   * OPTION_RAPID_COMMIT(14) {{bibref|RFC3315|Section 22.14}} in Solicit
   * messages.
   *
   * @since 2.2
  ...
  /**
   * Set the enables or disables inclusion of the ''Rapid Commit'' option
   * OPTION_RAPID_COMMIT(14) {{bibref|RFC3315|Section 22.14}} in Solicit
   * messages.
   *
   * @since 2.2
  ...
  /**
   * Get the when set to {{true}}, the {{object}} will renew its DHCPv6-supplied
   * information (i.e. the CPE will do a renew or information request as needed,
   * updating both stateful and stateless parameter values discovered by this
   * Client instance).
   *
  ...
  /**
   * Set the when set to {{true}}, the {{object}} will renew its DHCPv6-supplied
   * information (i.e. the CPE will do a renew or information request as needed,
   * updating both stateful and stateless parameter values discovered by this
   * Client instance).
   *
  ...
  /**
   * Get the t1 value, in seconds, that the client SHOULD use when sending IA
   * options, e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
   * {{bibref|RFC3633|Section 10}}.
   * <p>
   * A value of -1 indicates that no T1 value is specified.
   *
  ...
  /**
   * Set the t1 value, in seconds, that the client SHOULD use when sending IA
   * options, e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
   * {{bibref|RFC3633|Section 10}}.
   * <p>
   * A value of -1 indicates that no T1 value is specified.
  ...
  /**
   * Get the t2 value, in seconds, that the client SHOULD use when sending IA
   * options, e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
   * {{bibref|RFC3633|Section 10}}.
   * <p>
   * A value of -1 indicates that no T2 value is specified.
  ...
  /**
   * Set the t2 value, in seconds, that the client SHOULD use when sending IA
   * options, e.g. OPTION_IA_NA {{bibref|RFC3315|Section 22.4}} and OPTION_IA_PD
   * {{bibref|RFC3633|Section 10}}.
   * <p>
   * A value of -1 indicates that no T2 value is specified.
  ...
  /**
   * Get the {{list}} The options that the client is able to process in server
   * responses. This list MUST include both top-level and encapsulated options,
   * e.g. if the client is able to process OPTION_IA_NA (3) with an encapsulated
   * OPTION_IAADDR (5), the list would be expected to include both 3 and 5.
   *
  ...
  /**
   * Set the {{list}} The options that the client is able to process in server
   * responses. This list MUST include both top-level and encapsulated options,
   * e.g. if the client is able to process OPTION_IA_NA (3) with an encapsulated
   * OPTION_IAADDR (5), the list would be expected to include both 3 and 5.
   *
  ...
  /**
   * Get the {{list}} An ordered list of the top-level options (i.e. not
   * encapsulated options) that the client will explicitly request from the
   * server.
   * <p>
   * This parameter is intended only for options that are not necessary for the
   * basic operation of the protocol, and are not modeled elsewhere. For
  ...
  /**
   * Set the {{list}} An ordered list of the top-level options (i.e. not
   * encapsulated options) that the client will explicitly request from the
   * server.
   * <p>
   * This parameter is intended only for options that are not necessary for the
  ...
  /**
   * Get the this is a transitory table that lists the discovered DHCPv6 servers
   * (it does ''not'' model a local DHCP server). Table entries are
   * automatically created to correspond with these servers. However, it is a
   * local matter to the CPE when to delete old table entries.
   *
   * @return the value
   */
  ...
  /**
   * Set the this is a transitory table that lists the discovered DHCPv6 servers
   * (it does ''not'' model a local DHCP server). Table entries are
   * automatically created to correspond with these servers. However, it is a
   * local matter to the CPE when to delete old table entries.
   *
  ...
  /**
   * Get the top-level options and option values (including any encapsulated
   * options) that the client will send to the server.
   * <p>
   * This table is intended only for options that are not part of the basic
   * operation of the protocol, and whose values are simple, do not often change
  ...
  public Collection<SentOption> getSentOptions() {
    if (this.sentOptions == null) {
      this.sentOptions = new ArrayList<>();
    }
    return sentOptions;
  }

  /**
   * Set the top-level options and option values (including any encapsulated
   * options) that the client will send to the server.
   * <p>
   * This table is intended only for options that are not part of the basic
   * operation of the protocol, and whose values are simple, do not often change
  ...
  /**
   * Get the this is a transitory table that lists all the options received from
   * all servers. Table entries are automatically created to correspond with
   * received options. However, it is a local matter to the CPE when to delete
   * old table entries.
   * <p>
   * If the same option is received multiple times, whether from one or more
  ...
  public Collection<ReceivedOption> getReceivedOptions() {
    if (this.receivedOptions == null) {
      this.receivedOptions = new ArrayList<>();
    }
    return receivedOptions;
  }

  /**
   * Set the this is a transitory table that lists all the options received from
   * all servers. Table entries are automatically created to correspond with
   * received options. However, it is a local matter to the CPE when to delete
   * old table entries.
   * <p>
